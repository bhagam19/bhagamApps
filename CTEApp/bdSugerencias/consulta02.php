<?php
	...
	switch ($cnt) {
		case 1: //Sede
			$consulta=mysqli_query($conexion,"SELECT DISTINCT idEstudiante FROM estSugeridos WHERE idEstudiante=".$cod);
		...
		case 8:
			$razones=array();
			$consulta=mysqli_query($conexion,'SELECT DISTINCT idRazon FROM estSugeridos WHERE idEstudiante='.$estudiante.' AND idArea='.$area.' AND docNominador='.$docNominador);
			while($fila = mysqli_fetch_array($consulta)){
		...
		case 9:
			$subrazonesBD=array();					
			$sql=mysqli_query($conexion,'SELECT DISTINCT idSubrazon FROM estSugeridos  WHERE idEstudiante='.$estudiante.' AND idArea='.$area.' AND docNominador='.$docNominador.' ORDER BY idSubrazon');
			...
			$consulta = mysqli_query($conexion,'SELECT  DISTINCT idRazon FROM estSugeridos WHERE idEstudiante='.$estudiante.' AND idArea='.$area.' AND docNominador='.$docNominador.' ORDER BY idRazon');
			$respuesta='';
			// $cont=1;
			while($fila = mysqli_fetch_array($consulta)){
