<html style="background:#F2F2F2">
	...
	<script>
		function showHint(str) {
			...
			if (str.length == 0) { 
		        ...
		        xmlhttp.send();
		        document.getElementById("hint").innerHTML = "Docentes ya registrados: <br>"+xmlhttp.responseText;
		    }
		}
		...
		function registrarDocente(){
			...
			if(apellidos===""){
				...
				var xmlhttp = new XMLHttpRequest();
		        xmlhttp.open("GET", "../bdDocentes/crearDocente.php?apellidos="+apellidos+"&nombres="+nombres, false);
	        	xmlhttp.send();
			    if(xmlhttp.responseText.trim()=="si"){
			    	alert("El docente "+apellidos+" "+nombres+" fue registrado con exito.");
		        	...
		        	alert("El docente "+apellidos+" "+nombres+" ya está registrado.")
		        	document.getElementById("apellidos").value="";
		        	document.getElementById("nombres").value="";
		    		document.getElementById("apellidos").focus();
		    		return false;
					...
					<div class="formularioReservaciones" >
						<div>
    						<table border="1" style="width:60%;!important">
    							<caption> AGREGAR NUEVO DOCENTE</caption>
    							...
    							<tbody style="background:lightgreen">
    					        	<tr>
        								<td style="text-align:left">
        									<input type="text" name"apellidos" id="apellidos" style="width:250px" onkeyup="showHint(this.value)">
        								</td>
        								<td style="text-align:left">
        									<input type="text" name"nombres" id="nombres" style="width:250px" onkeyup=this.value=this.value.toUpperCase();>
        								</td>
    								</tr>
    								<tr>
    									<td colspan="2"><span id="hint" > </span></td>	
    								</tr>
    								<tr>
    									<td colspan="2" style="text-align:center">
    										<input value="Registrar" type="button" onclick="registrarDocente()" style="width:70px">
    					...
    					<div style="height:380px;overflow:auto">
        					<table class="tabla" width="100%" >
        						<caption>DOCENTES REGISTRADOS ("Click" para actualizar.)</caption>
    							<thead>
    								<tr>
    									<th>ID</th>
    									<th>APELLIDOS</th>
	...
	<script>
		...
		function actualizarRegistro(id,valor,campo){
			...
			var xmlhttp = new XMLHttpRequest();
			xmlhttp.open("GET", "actualizarDocente.php?id="+id+"&valor="+valor+"&campo="+campo, false);
			xmlhttp.send();
