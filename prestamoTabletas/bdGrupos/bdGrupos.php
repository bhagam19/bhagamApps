<html style="background:#F2F2F2">
	...
	<script>
		...
		function registrarGrupo(){
			...
			if(grupo===""){
				...
				document.getElementById("grupoN").focus();
				return false;
			}else{
				var xmlhttp = new XMLHttpRequest();
		        xmlhttp.open("GET", "../bdGrupos/crearGrupo.php?grupo="+grupo, false);
	        	xmlhttp.send();
			    if(xmlhttp.responseText.trim()=="si"){
			    	alert("El grupo "+grupo+" fue registrado con exito.");
					xmlhttp = new XMLHttpRequest();
					xmlhttp.open("GET","actualizarActualizable.php",false);
					xmlhttp.send();
					document.getElementById("grupoN").value="";
					document.getElementById("actualizable").innerHTML=""
					document.getElementById("actualizable").innerHTML=xmlhttp.responseText.trim();	
		        }else{
		        	alert("El grupo "+grupo+" ya está registrado.")
		        	document.getElementById("grupoN").value="";
		    		document.getElementById("grupoN").focus();
		    		return false;
		        }    
					...
					<div class="formularioReservaciones" >
						<div>
    						<table border="1" style="width:60%;!important">
    							<caption> AGREGAR NUEVO GRUPO</caption>
    							...
    							<tbody style="background:lightgreen">
    					        	<tr>
        								<td style="text-align:left">
        									<input type="text" name"grupoN" id="grupoN" style="width:250px" onkeyup="showHint(this.value)">
        									<br><span id="hint"> </span>
        								</td>
    								</tr>
    								<tr>
    									<td colspan="2" style="text-align:center">
    										<input value="Registrar" type="button" onclick="registrarGrupo()" style="width:70px">
    					...
    					<div style="height:380px;overflow:auto">
        					<table class="tabla" width="100%" >
        						<caption>GRUPOS REGISTRADOS ("Click" para actualizar.)</caption>
    							<thead>
    								<tr>
    									<th>ID</th>
    									<th>GRUPO</th>
	...
	<script>
		...
		function actualizarRegistro(id,valor,campo){
			...
			var xmlhttp = new XMLHttpRequest();
			xmlhttp.open("GET", "actualizarGrupo.php?id="+id+"&valor="+valor+"&campo="+campo, false);
			xmlhttp.send();
		...
		function eliminarRegistro(id){
			/*alert(id);*/
			var xmlhttp = new XMLHttpRequest();
			xmlhttp.open("GET","eliminarGrupo.php?id="+id,false);
			xmlhttp.send();
